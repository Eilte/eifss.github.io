/*
 * @Author: Han
 * @Date: 2021-11-01 10:20:48
 * @LastEditors: Han
 * @LastEditTime: 2021-11-01 10:48:44
 * @FilePath: \ipfs\static\file.js
 */
$(() => {/* 临时粘贴上传 */$(document).on('paste', event => {let clipboardData = event.clipboardData || window.clipboardData || event.originalEvent.clipboardData;/* 判断是否支持粘贴上传 */if (!clipboardData || !clipboardData.items) return alert('当前浏览器不支持粘贴上传');let items = clipboardData.items;let file = null;/* 判断剪切板的内容是否是桌面类型的文件 */if (items.length === 0) return alert('剪切板内无内容或不支持桌面文件');/* 开始循环剪切板的内容，判断是否是文件类型，如果是文件类型，则push */for (let i = 0; i < items.length; i++) {if (items[i].type.indexOf('image') !== -1) {file = items[i].getAsFile();}}if (!file) return alert('剪切板内无内容或不支持桌面文件');upload(new Array(file));});/* 点击上传 */$('.upload .content').on('click', function () {$('#file').click();});/* 监听点击上传 */$('#file').on('change', () => {upload($('#file')[0].files);});/* 拖拽上传 */$('#dragbox').on('dragover', e => {e.preventDefault();});$('#dragbox').on('dragenter', e => {e.preventDefault();$('.upload').addClass('dragenter');});$('#dragbox').on('dragleave', e => {e.preventDefault();$('.upload').removeClass('dragenter');});$('#dragbox').on('drop', e => {e.preventDefault();$('.upload').removeClass('dragenter');let files = e.originalEvent.dataTransfer.files;upload(files);});console.group("%c \u4F5C\u8005\u4FE1\u606F", "color: #fadfa3; background: #030307; padding:5px");console.log("%c \u4e00\u652f\u7a7f\u4e91\u7bad\uff0c\u5343\u519b\u4e07\u9a6c\u6765\u76f8\u89c1\uff01 My Name's \u97E9\u5C0F\u97E9 QQ\uff1a1655466387", "color: #fadfa3; background: #030307; padding:5px");console.log("%c \u97E9\u5C0F\u97E9\u535A\u5BA2 Www.Vvhan.Com", "color: #fadfa3; background: #030307; padding:5px");console.log("%c \u97E9\u5C0F\u97E9API\u63A5\u53E3 Api.Vvhan.Com", "color: #fadfa3; background: #030307; padding:5px");console.log("%c \u6B22\u8FCE\u524D\u6765\u56F4\u89C2\u3001\u5410\u69FD\u3001\u70B9\u8D5E\u3001\u5B66\u4E60......", "color: #fadfa3; background: #030307; padding:5px");console.groupEnd();console.group("%c \u4E00\u8A00", "color: #ffffff; background: #6666FF; padding:5px");console.log("%c \u96BE\u5FD8\u548C\u96BE\u8FC7\u603B\u90FD\u662F\u4F1A\u6709\u7684\u5427", "color: #ffffff; background: #6666FF; padding:5px");console.log("%c \u90A3\u4E9B\u4EBA\u548C\u4E8B\u60C5\u603B\u90FD\u662F\u4F1A\u6709\u7684\u5427", "color: #ffffff; background: #6666FF; padding:5px");console.log("%c \u660E\u4EAE\u4E0E\u6666\u6697\u603B\u90FD\u662F\u4F1A\u6709\u7684\u5427", "color: #ffffff; background: #6666FF; padding:5px");console.groupEnd();/* 上传函数 */function upload(files) {for (let i = 0; i < files.length; i++) {var animateimg = files[i].name;var imgarr = animateimg.split('\\');var myimg = imgarr[imgarr.length - 1];var houzui = myimg.lastIndexOf('.');var ext = myimg.substring(houzui, myimg.length).toUpperCase();var file = files[i];if (!file) {return false;}var fileSize = file.size;var maxSize = 5242880 * 20 * 2;if (parseInt(fileSize) >= parseInt(maxSize)) {parent.alert('上传的文件不能超过' + maxSize / 1024 / 1024 + 'MB');return false;} else {
...
// Ly8gYXBpID0gJ2h0dHBzOi8vaXBmcy5pbmZ1cmEuaW86NTAwMS9hcGkvdjAvYWRkP3Bpbj10cnVlJwovLyBhcGkgPSAnaHR0cHM6Ly9odHRwczovL2lwZnNhcGkuZ2xpdGNoLm1lL2FwaS92MC9hZGQ/cGluPXRydWUnCi8vIGFwaSA9ICdodHRwczovL2lwZnMuc3RhZ2luZy5pbmZ1cmEub3JnOjUwMDEvYXBpL3YwL2FkZD9waW49dHJ1ZSc=
